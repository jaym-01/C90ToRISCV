...
for DRIVER in compiler_tests/${SPECIFIC_FOLDER}/*_driver.c; do
    ...
    timeout --foreground 15s riscv64-unknown-elf-gcc -o "${OUT}.o" -c "${OUT}.s" 2> "${LOG_FILE_BASE}.assembler.stderr.log" > "${LOG_FILE_BASE}.assembler.stdout.log"
    if [ $? -ne 0 ]; then
    ...
    timeout --foreground 15s riscv64-unknown-elf-gcc -static -o "${OUT}" "${OUT}.o" "${DRIVER}" 2> "${LOG_FILE_BASE}.linker.stderr.log" > "${LOG_FILE_BASE}.linker.stdout.log"
    if [ $? -ne 0 ]; then
